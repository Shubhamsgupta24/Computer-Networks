...
def client(server_address, server_port):
    ...
    while True:
        ...
        if curr_pkt_corrupt_prob>CORRUPT_PACKET_THRESHOLD:
            print(f'Corrupted ACK Packet for packet no {sequence_number} recieved from server.Resending the packet with packet no {sequence_number}.\n')
            
        elif response=='NAK':
            print(f'ACK Packet for packet no {sequence_number} recieved from server for wrong packet received at server.Resending the packet with packet no {sequence_number}.\n')
